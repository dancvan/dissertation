...
We exploit the chosen optic / disk symmetry about the polar angle ($\partial^{2} V / \partial \theta^{2}$) and compute for the longitudinal ($z$) and radial ($r$) coordinates with the use of the appropriate Laplacian:
\\
$$ \partial V / \partial \theta = 0 $$
...
\begin{equation}
    \frac{\partial}{\partial r} \rightarrow \frac{f(r + h, z) - f(r - h, z )}{2h}
\end{equation}

Stencil:
\\
$$ \frac{1}{r} \bigg[-\frac{1}{2} \quad  0  \quad \frac{1}{2} \bigg] $$

\subparagraph*{Term (b)}
Second derivative approximation, we use the standard 2d laplace stencil

\begin{equation}
    \frac{\partial}{\partial r^2} \rightarrow \frac{f(r+h, z) - 2 f(r, z) + f(r-h, z)}{h^2}
\end{equation}

Stencil: 
\\
$$ \big[ 1 \quad -2 \quad 1 \big] $$

\subparagraph*{Term (c)}
Equivalent to the second derivative approximation used above:

\begin{equation}
    \frac{\partial}{\partial z^2} \rightarrow  \frac{f(r, z+h) - 2 f(r, z) + f(r, z-h)}{h^2}
\end{equation}
...
To build the stencil terms at the boundaries, we look at the specialized finite difference condition @ $ r = 0 $:

$$ \frac{\partial V}{\partial r} \rightarrow  [ \quad 0 \quad 0 \quad ] $$

$$ \frac{\partial^2 V}{\partial r^2} = \frac{V(r+h,z) - 2 V(r, z)}{h}

we taylor expand our function:

$$ V \approx V_0  + C_1 r +  C_2 r^{2} + \mathcal{O}(r^4) $$

Symmetry about the origin imposes an even functional form of $V$, and $C_1 = 0$:

$$ \frac{\partial V}{\partial r} \approx  2 C_2 r  + \mathcal{O}(r^3) $$

$$ \frac{1}{r} \frac{\partial V}{\partial r} \approx  2 C_2  + \mathcal{O}(r^2) $$

$$ \frac{\partial^2 V}{\partial r^2} \approx 2 C_2 + \mathcal{O}(r^2) $$

Substituting back into equation \autoref{eq:cyllap_xpand} :
\begin{equation}
    \frac{\partial^2}{\partial z^2} + 4 C_2 = 
\end{equation}
