...
\noindent \textcolor{red}{Detail on some prior knowledge of $f \leq f_\mathrm{max}$?}
...
\end{center}
\caption{Figure of the beam propogation axis with respect to the AlGaAs/GaAs crystal axes (not final version). Within the [100] plane the AlGaAs coating is grown with a flat indicator that draws a line within the [0-11] plane where the bisecting vector of the plane normal points towards the sample center. The axis formed by the [100] coincides with the optical axis (z-axis). The polarizations of incident and reflected beam oscillate along vectors within the plane formed by the normal of that axis.}
\label{fig:algaas_coords}
\end{figure}

Up until this point we have discussed three different coordinate axes: the crystal axis (formally indicated by Miller indices), the principal dielectric axis (coordinates based in diagonalization of the indicatrix), and an optical axis (when considering a desired (laser) light propogation). We assert an optical axis with linearly polarized light with variable s and p contributions.
...
With our coordinate considerations and established beam axis, it is now worth considering the influence of a isotropic white noise field ($E_n = [E_{nx},E_{ny},E_{nz}]$):
...
Assuming $E_n$ is small, the indicatrix change of $E_{nx}$ and $E_{ny}$ relative to $E_{ny}$ (as seen by the beam polarization) will be small ($r_{41}E_{n(x/y)} \ll r_{41}E_{nz}$). After diagonalizing with relevant terms in the tensor we are left with the following eigenindices:
...
The Pound-Drever-Hall technique, originally imagined for laser frequency stabilization to an ultra-stable length reference, allows the tracking of the linear phase response of an input carrier field through the resonance of said reference. The ability for the servo to track said response across the cavity resonance fully utilizes the ability of an optical cavity to act as a length / frequency discriminator. Cavity offset locks also provide a linear response in intensity which may be adequate for some applications but with reduced sensitivity due to the required power reduction by operating off resonance.
\begin{figure}[H]
\includegraphics[width=\textwidth]{figs/ALGAAS/DC_power_cav_resonance.png}
\caption{Reflected and transmitted power around resonance. Phase as dotted line?}
\label{fig:cav_length_response_DCpow}
\end{figure}
Measurements of the phase can be achieved by beating a separate optical field with a known frequency separation from the carrier at the cavity input. The PDH servo offers a way to avoid complicated phase-locked two laser configurations by imposing a phase modulation on the carrier field via electro-optical modulator. The phase modulation onto the carrier field is mathematically and physically equivalent to imposing separate optical fields (sidebands) which in most cases do not resonate in the optical cavity of interest. The modulation imposed onto the carrier is by a physically equivalent process to that mentioned in section (?). The simplified anatomy of the EOM is split into three components, a crystal used to impose the modulation, capacitor plates sandwiching the crystal to introduce a controlled electric field across, and an oscillator / signal generator used to drive the capacitor at a specified frequency $\Omega$ and a voltage amplitude associated to the phase amplitude known as the modulation depth ($\beta$).
...
Setting a photodiode of area ($A_\mathrm{PD}$) in reflection of the cavity reflection coefficient of $r_\mathrm{cav}(\omega,L)$, we measure the power from the field:
...
The two trailing terms in the above equation for $P_\mathrm{refl}$ generate a beat frequency term between the carrier and lower and upper sidebands. 
