...
\end{equation}
For our problem we assume no free charge, but the fused silica substrate with the AlGaAs coating presents dielectric material between the plates. Boundary conditions are expressed in terms of the differential plate potential $V = V_{+} - V_{-}$ plate potentials so it is natural to first solve for potential ($V$) for all relevant system coordinate points. We exploit cylindrical symmetry with the optic and plate geometry in the radial coordinate ($r$) and use the appropriate Laplacian: 
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{figs/ALGAAS/longitudinal_pockels_cell_bc.pdf}
...
\noindent Inspired by the second-order elliptic equation, operators are modified to accomodate for polar coordinate symmetry that is offered by using \autoref{eq:cyllap}.

\[ 
\mathcal{L}_{cyl} = 
\begin{bmatrix} \,
\mathcal{K}_{n} & \mathcal{I}_{n} & O_{n} & O_{n} & O_{n} & \cdots & \cdots & \cdots & \cdots & \cdots & O_{n}\\
...
O_{n} & O_{n}  & \mathcal{I}_{n} & \mathcal{K}_{n} & \ddots & O_{n} & O_{n} & O_{n} & O_{n} & O_{n} & \vdots\\
...
\vdots & O_{n}  & O_{n}  & O_{n} & O_{n} & O_{n} & \ddots & \mathcal{K}_{n} & \mathcal{I}_{n} & O_{n}  & \vdots\\
...
O_{n}      & \cdots & \cdots & \cdots & \cdots & \cdots & O_{n} & O_{n} & O_{n} & \mathcal{I}_{n} & \mathcal{K}_{n}
\end{bmatrix} 
\]
...
With G representing the open loop gain ($\mathrm{G} = \mathrm{A}_1 \mathrm{F} \mathrm{S} \mathrm{L}$ ) 

The feedback signal transforms into:
