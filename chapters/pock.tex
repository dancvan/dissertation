...
\noindent Where we have approximated with the Jacobi-Anger expansion utilizing Besssel functions of the first kind ($J_n(x)$):
...
\begin{equation}\label{eq:partial_M}
...
The above representing that the collective differential phase will essentially manifest as a sum of these phase components once computed. This explicit perturbed phase for the electro-optic effect ($\partial n/\partial E$) is found when:
...
An easier computation is found when relying on the relationship between the transmission (t) and reflectivity (r) to a general transfer matrix (in our case M):

\begin{equation}
	\begin{bmatrix} 1 \\ r \end{bmatrix} \begin{bmatrix} M_{11} & M_{12} \\ M_{21} & M_{22} \end{bmatrix} = \begin{bmatrix} t \\ 0\end{bmatrix}
\end{equation}

And using this relation, if we differentiate the reflectivity with respect to $\phi_k$:
...
and normalizing this by the total reflectivity and taking the imaginary component as indicated in \ref{eq:partial_M}: 

\begin{equation}
	\mathrm{Im} \bigg(\frac{1}{r} \frac{\partial r}{\partial \phi_k} \bigg) =  \frac{\phi_c}{\phi_k} = \bigg( \frac{1}{M_{21}} \frac{\partial M_{21}}{\partial \phi_k} - \frac{1}{M_{22}}\frac{\partial M_{22}}{\partial \phi_k} \bigg) 
\end{equation}
...
To estimate how much DARM coupling can occur, we use a measured field spectra acquired from installed electric field meters located within LIGO Hanford Observatory ETMX and ETMY vacuum chambers. Taking the upper and the lower EFM measurements from ~\cite{efmlog} $.3\; [\mathrm{V}/\mathrm{m}/\sqrt{\mathrm{Hz}]}$ @ 60 Hz and $4\times10^{-3}\; [\mathrm{V}/\mathrm{m}/\sqrt{\mathrm{Hz}]}$ @ 4kHz.
...
The significantly improved thermal noise performance of $\gaas$/$algaas$ HR coatings make them prime candidates for current and future gravitational wave detectors. The investigated
