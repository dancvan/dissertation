...
\begin{figure*}
     \begin{subfigure}{\includegraphics[width=.40\textwidth]{figs/ALGAAS/pockels_cell_l.png}}
     \end{subfigure}
     \hfill
     \begin{subfigure}{\includegraphics[width=.40\textwidth]{figs/ALGAAS/pockels_cell_t.png}}
     \end{subfigure}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=.75\textwidth]{ALGAAS/opt_layout_a.jpeg}
\end{center}
...
The sensing filter electronics are composed of a single element photodiode (from?) with a QE of ? and the response found in appendix ?. The signal is then mixed with a 25 MHz oscillator phased 180 degrees ? m of cable so the measured beat signal while sweeping through resonance generates the \href{fig:pdh_error}{PDH error signal profile}.
\begin{itemize}
\item 25 MHz RFPD
\begin{itemize}
...
\begin{figure}[H]
\begin{center}
\includegraphics[width=.7\textwidth]{ALGAAS/assemblies/disk_electrode_concept.png}
\end{center}
...
\caption{$|E_z|$ screened by the coating and immediately outside AlGaAs coating. \textcolor{red}{Needs to be updated with more current voltage settings}}
\label{fig:Ez}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=.75\textwidth]{figs/ALGAAS/tfs/HVA_TREK1010BHS_1260V_out.png}
...
\section{Results}
Measurements made with A major challenge in the design choice made for this experiment is the differential noise when mounting the optics in accessible non-conductive materials. Most commercial optical mounts are conductors which proved to be challenging when attempting to find a mounting solution while reducing the non-normal field gradients within the coating volume of interest within the sample. For this reason, efforts were focused on developing a suitable mounting solution that would provide isolation from any uncontrolled field magnitudes while providing one normally incident on the surface, with enough strength and uniformity across the beam area to extract a measurement of the differential length change from the Pockels effect. The mounts are separated by large geometric and / or material differences.
...
With the the above servo electronics transfer functions, we calibrate the measurement into a differential length:
\begin{equation}
...
\textcolor{red}{Difficulty locking from lack of angular control}

\subsubsection{Assembly 1}

\begin{figure}[H]
\centering
\includegraphics[width=.75\textwidth]{figs/ALGAAS/assemblies/assembly1/assembly_1_dissassembled.png}
...
\caption{Figure that will include the all displacement noise floors, (pockels estimate)*(poisson calculator estimate)*(HVA drive frequency dependence), and the drive coupled measurement \textcolor{red}{figure size needs to be increased}}
