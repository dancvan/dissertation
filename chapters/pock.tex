...
Both forms of the induced birefringence can be incorportated into the following condensed form \cite{nye}:
\begin{equation}\label{eq:indicgen}
	\bigg( \frac{1}{\Delta n^2} \bigg)_{ij} = r_{ijk}E_k + p_{ij \alpha \beta} \epsilon_{\alpha \beta}
\end{equation}

Where $\alpha$ and $\beta$ are dummy indices that allow the connection between elasto-optic ($p$) tensor to the stress tensor ($\epsilon$) via the stress-strain / piezoelectric relations:
