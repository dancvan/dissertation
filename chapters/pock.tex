...
\noindent Where $\phi$ and $E_o$ are the Poisson ratio and Young's modulus respectively, and $O(\frac{r_o}{R})$ is a correction term as a function of the small beam radius ($r_o$) relative to the mirror radius ($R$).
...
\noindent For a z-wavevector $E_{[100]} = E_{[010]} = 0$ and $E_{[001]} \neq 0$, computing the eigenvalues and eigenpolarizations will lead us to the coorelated eigenindices:
...
\end{equationappendix:ifo_configs_code}}

\noindent The eigenindices ($n_\alpha = n_{[110]}$, $n_\beta = n_{[-110]}$) are therefore:
...
\subsection{Miller indices for highly reflective $\gaas$/$\algaas$ coatings}
\begin{figure}[!hb]
...
\noindent Assuming we are operating in a coordinate system suggested in \autoref{fig:algaascoords}, which plane is impacted by some isotropic $E_\mathrm{noise}$? Revisiting the \autoref{eq:zindicatrix} we can see that non-zero x, y, and z components would only couple to the input beam polarization oscillating in the x-y plane on account of $E_z$ and as demonstrated gives us the ability to diagonalize the indicatrix tensor when realizing the case $E_x << E_z$ and $E_y << E_z$.

\subsubsection*{Analytic estimate}
Fejer and Bonilla take on an analytical approximation when finding the impact of the electric field to the change in phase of the light through a crystalline anisotropic thin film ($\lambda/4$) stack \cite{bonillafejer, fejer_estimate}:
...
Sensing electronics are composed of a single element photodiode mounted to a tranimpedance amplifier PCB that redirects photocurrent to DC and RF outputs. The RF path is constructed so the RF signal is boosted before being passed to the FSS mixer where it is demodulated by mixing the 25 MHz oscillator phased 180 degrees (with variable cable length). Once demodulated, the measured beat signal while sweeping through resonance generates the PDH error signal profile \autoref{fig:pdherr}.
\begin{itemize}
\item 25 MHz RFPD
\begin{itemize}
...
\begin{figure}[!ht]
    \begin{subcaptiongroup}
	    ...
	    \phantomcaption\label{splitresonance}
	    \includegraphics[width=.52\textwidth]{figs/ALGAAS/fas_axes.png}
	    \phantomcaption\label{fasaxes}
    \end{subcaptiongroup}
