...
The transient ring heater actuation from a radially symmetric thermal aberration ($\Psi(t,r)$) is realized \cite{Ramette:16}:
\begin{equation}
...
\begin{figure}[H]
 ...
 \caption{ITMY thermo-optic response to a 3.13 Watt power reduction to the top and bottom ring heater elements. It's after $\approx$ 12 hours after a ring heater power step was made do you start to see a small enough differential defocus ($\frac{\mathrm{d} \alpha_\mathrm{sp}}{\mathrm{dt}}$) and can assume a steady thermal lens.}
 \label{fig:meas}
\end{figure}

The measured thermo-optic step response exhibits a differential defocus for $\approx$ 12 to 15 hours after the ring heater power has been changed; and with a large enough power step, adjustments to ring heater power can stall precious detector observing/comissioning time with differential mode matching in the DRFPMI. Reducing the thermo-optic time constant is possible by applying a real time digital filter to the ring heater power control.
