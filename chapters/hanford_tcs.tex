...
The prescription for creating such a filter is realized through inverting the well known step response. But this alone can generate control instability which can be easily remedied with additional low passing at high frequency.
...
Another symptom of resonant / high power optical cavities is an opto-mechanical phenomenon known as parametric instability (PI); where test mass mechanical modes and higher order optical modes form a positive feebackback loop that can drive the detector to lockloss. Passive methods of control by way of acoustic mode dampers (AMD) showed significant reduction of problematic mechanical modes though some proved to remain problematic; requiring manual intervention by way of adjusting test mass geometry to disrupt these persistent PIs. With the test mass geometry directly coorelating to the thermo-optic test mass response, shifting from runaway PIs through slight ring heater adjustments plus DTC has been presented as a feasible solution \cite{Hardwick_2020}. 
