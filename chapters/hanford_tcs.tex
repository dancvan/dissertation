...
\section{Motivation}
As seen in ~\autoref{sec:detcon}, increasing detector input power leads to a direct sensitivity increase to gravitational waves. Though, with increased input power to a DRFPMI, more light is coupled into the Fabry-P\'{e}rot cavity arms; and even using optics with ultra-low absorption ($\approx 400 \; \mathrm{ppb} \pm 150 \; \mathrm{ppb}$ \textcolor{red}{alog ? or point absorber paper}) there still are induced thermo-optic effects, and with a designed circulating arm power of 750 kW there is a produced substrate lens with a relatively smaller radius of curvature. These thermal aberrations produce mode mismatch, which accumulate optical loss throughought GWDs and reduce sensitivity two-fold: loss of power to your readout, and reduced efficacy of the current solution to reduce the quantum noise limit (squeezing).
\\
...
The system is comprised of an adaptive optics system that uses four Hartmann wavefront sensors (HWS) providing near-real-time data of thermal distortions, and is combine with actuators of two forms: annular ring heaters and CO2 lasers on select core optics. 

\subsubsection{Sensing Optical Path Distortion}
Measuring thermal distortions from both carrier as well as thermal actuators is done using a set of four Hartmann wavefront sensors; each one measuring the differential optical path length at each FP cavity test masses. The probe beam (with differing wavelengths of 800 nm and 833 nm for the X and Y arms to mitigate crosstalk between sensing chains) makes a double pass through the test mass mirror substrate for all cavity mirrors and maps the HR mirror surface; while making an additional double pass at the input test mass sensors through a compensation plate (CP) installed between the BS and the input test mass mirrors.

\subsubsection{Actuation}
Both ITMs and ETMs utilize mode matching actuation, though they are not prescribed equal treatment; all Fabry-P\'{e}rot cavities do possess negative lens actuation in the form of a wound nichrome wire annulus outlining the outer barrel of the test masses while CO2 actuation is applied only to a compensation plate (CP) \footnote{CO2 laser actuation is not applied directly to the test mass surface to avoid imparting laser noise from the actuating beam directly to the FP test mass} located between the beam splitter (BS) and the input test mass (ITM) to counter or replicate the substrate lens via negative or positive actuation respectively~\cite{aasi:2015}.
...
The gaussian bean ~\cite{brooks:aigwd2019}
