...
\begin{figure}[H]
	...
	\caption{A simple controls diagram depicting the concept of a RH control pre-filter (\textbf{G(s)}) taking a step input (\textbf{u(t)}) and transforming it before reaching the thermo-optic plant (\textbf{H(s)}).}
...
\begin{figure}[H]
    \includegraphics[width=\textwidth]{TCS/NB_1235813418}
    \caption{Figure of DARM noise during O3a. Beneficial features of interferometry, i.e. common mode noise rejection require adequate mode overlap at the beam splitter. Destruction of such symmetry as mode overlap proportionally destroys such benefits and demonstrate a presence in DARM}    
...
Higher order modes power at the anti-symmetric port obscure the single element photodiode from measuring a pure TEM00 mode. 
