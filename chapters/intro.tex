...
Initially used by Michelson and Morley to test for the existence of luminiferous aether; the optical schema and detection philosophy demonstrate an extraordinary potential for measuring the quadrapole moment of a gravitational wave and make it a prime candidate as a means of detection. The perpendicular beam paths set at the beamsplitter and two end mirrors create an interference condition between the returning beams which are disrupted by differential path length changes between the beam paths and tracked at the (anti-symmetric) detection port:

\begin{equation}
	P_\mathrm{out} = \frac{P_\mathrm{in}}{2} \bigg[1+\mathrm{cos}\Big(\frac{4\pi}{\lambda} (L_x - L_y)\Big) \bigg]
\end{equation}

The Michelson does well registering a microsocopic length change on a sub-wavelength level and is more appropriately discussed as a differential phase response. From here the influence of a time-dependent metric perturbation ($h(t)$) on the interferometer is evaluated:
...
And inserting this differential phase back into our power measurement we are left with the following:
