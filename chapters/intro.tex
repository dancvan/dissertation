...
Light enters the cavity only after passing the input mirror with a field amplitude reduced by the mirror reflection coefficient. Quick observation may suggest that this isn't quite useful as we have already reduced the power for every entering phasefront. Though with an integer number of wavelengths between these two mirrors and their coherent addition with the input beam, achieves cavity resonance. The power stored between the two cavity mirrors coorelates with how long a phasefront remains stored in an optical cavity through its finesse (\mathscr{F}). This ``cavity storage time" ($\tau_s \appropto L r_1 r_2$) translates as a length elongation with the phasefront travel history encoded in the arrival time of its photons back at the beam splitter. A cavity of length $L$ and comprised of an input coupler ($r_1$,$t_1$) and end mirror ($r_2$, $t_2$) yield the following effective cavity reflection and transmission coefficients: 
...
The tuning of mirror positions for infared lasers with high reflectivity coefficients sets a condition for resonance that may require strict length tuning and maintenance $$\leq 1e-7$$ [m]. 

\begin{figure}[H]
\includegraphics[width=\textwidth]{ALGAAS/REFL_cav_intensity.pdf}
\caption{Reflected cavity intensity (I$_\mathrm{REFL}$) around resonance. The resonance peak full width half maximum is set by mirror reflectivities and succinctly quantified by cavity finesse ($\mathscr{F} = \frac{\mathrm{FWHM}_\mathrm{res}}{f_\mathrm{FSR}} = \frac{\pi \sqrt{r_1 r_2}}{1-r_1 r_2}$).}
...
Advanced LIGO, with it's 4km length and projected finesse of  coorelates to a storage time of ?.
...
As seen in the field expression this profile evolves with the longitudinal propogation of the beam and introduces another means of introducing harmful assymetry between the interferometer arms. 
