...
The concept arose while Einstein dove deep into general relativity; a theory best summarized by the statement ``Space-time tells mass how to move; mass tells space-time how to curve"\cite{}. 
\textcolor{red}{Metric tensor}
\textcolor{red}{Refer to GR notes}

\subsection{Gravitational wave astronomy}
The story of gravitational wave detection is one with a history of troubled minds concerned by the lack of experiments to supplement the findings of general relativity. The literature provided by these individuals tells that fascinating and humbling story while providing important motivation and details on the trajectory of this field.
...
A design initially used by Michelson and Morley to refute the existance of aether (a now obsolete hypothesis suggesting light requires a medium in which to travel), the geometry and response to differntial arm movement may suggest it's apparent use for gravitational wave detection. The instrument, when operating on a dark fringe, places mirror postitions such that there is no light at the ant-symmetric port while the instant there is time dependent differential arm perturbation, the photodiode at the output registers a change in measured power:

\begin{equation}
	P_\mathrm{out} = \frac{P_\mathrm{in}}{2}(1+\mathrm{cos}2(k_x L_x -k_y L_y))
\end{equation}

Knowing the Michelson will register a diffential arm shift with adjusting fringe brightness, we zoom into the differential phase response of the Michelson say from a time-dependent metric perturbation:

\begin{equation}
\Delta \phi = \phi_x - \phi_y =  \int_{t-2L/c}^{t} \Omega \bigg[1 + \frac{1}{2}h(t)\bigg]dt - \int_{t-2L/c}^{t} \Omega \bigg[1 - \frac{1}{2}h(t)\bigg]dt \label{eq1}\tag{1} 
\end{equation}
