...
\\
In other words, there exists a wave solution with two separate transverse polarizations $h_{+}$ and $h_{\times}$ with a $45^{\circ}$ separation between them. Measurement and analysis of these waves allow astronomers to extract novel information related to the progenitors through testing various hypotheses pertinant to the violent dynamics of these systems. On September 14th, 2015 the first confirmed gravitational wave detection was made from a coalescing black hole pair 1.3 billion light years away, and since this first confirmed detection, other detectors (Virgo, KAGRA, etc.) have joined the GW detector network; continuing the search for novel events. Once confirmed they are added to the growing catalogue of astronomical events by providing further insight on multi-messenger events ~\cite{gw170817} and new observations of more exotic systems ~\cite{Nitz_2023}.


\newpage
\section{Detector configurations}
Motivation for the current ALIGO experimental schema frankly cannot be taken for granted by the author; the high number of subsystems can make it easy to get lost when working within specialized observatory operations. The goal of this section is to compile a brief narrative of the core detector mechanisms; motivating their inclusion with the hopes of developing a holistic view of the LIGO detection schema especially as it pertains to the body of this work. Most introductory detector configuration discussions start with the michelson interferometer and end at the dual-recycled Fabry-Perot Michelson interferometer; I follow in kind. Alongside the discussion are citations which can provide exceptional alternative and detailed explanations of the topics discussed.
...
\subsubsection{Gaussian beams, mode overlap, and contrast defect}
Up to this point, mentions of specific transverse phasefronts of the carrier has eluded the conversation. It is worthwhile to define this trait of the carrier with wavelength $\lambda$, and launched from a laser source along the propogation axis ($z$); that being the Gaussian beam (TEM00 mode):

\begin{equation}\label{eq:gaussian_beam}
E(r) = E_o \frac{\sqrt{[\lambda z_o] / \pi}}{W(z)}e^{-r^2 / W^2(z)} e^{-ikz - ik[r^2 / (2R(z))] + i \zeta}
\end{equation}

Where $E_o$ is the complex wave amplitude, $r = \sqrt{x^2 + y^2}$ defines transverse coordinates of a selected phasefront, $k$ is the wave number, $W(z)$ is the beam width, $R(z)$ is the beam radius of curvature, and $\zeta$ is the Gouy phase.

This evolution of the Gaussian beam wavefront adds another subtle requirement for interference at the beam splitter. As seen in the field expression this profile evolves with the longitudinal propogation of the beam and furthers the possability to introduce assymetries between the interferometer arms that effect interference. Two metrics are used to described how well this overlap can be planned with the mode overlap coefficient and performed interference with contrast defect:

\begin{equation}\label{eq:mode_overlap}
	\eta = \bigg|\int E_x E_y dA \bigg|^{2} \bigg/ (P_x P_y)
\end{equation}

Experimentally, successfully planned and performed mode overlap results in the high maximum power ($P_\mathrm{max}$) and a low minimum power ($P_\mathrm{min}$) at the anti-symmetric port; with the ideal visibility set as close as possible to unity ($\nu$).

\begin{equation}\label{eq:contrast_defect}
	\nu = \frac{P_\mathrm{max} - P_\mathrm{min}}{P_\mathrm{max} + P_\mathrm{min}}
\end{equation}

\textcolor{red}{It is also important to consider for any sufficiently long length, you will most likely limit your Michelson arm length or power at the anti-symmetric port assuming you don't have a large enough beamsplitter and/or focusing lenses} 
...
Light enters the cavity only after passing the input mirror with the incident field amplitude reduced by the mirror reflection coefficient. Quick observation may suggest that this isn't quite useful as we have already reduced the power for every entering phasefront. Though with an integer number of wavelengths of light between these two mirrors, the circulating beam coherently adds with the input, achieving resonance.  A cavity of length $L$ and comprised of an input coupler ($r_1$,$t_1$) and end mirror ($r_2$, $t_2$) yield the following effective cavity reflection and transmission coefficients: 
...
The tuning of mirror positions for infared lasers with high reflectivity coefficients set a resonance condition that requires strict length tuning and maintenance ($\leq 1\mathrm{e-}7$ [m]). 
