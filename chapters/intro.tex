...
Initially used by Michelson and Morley to test for the existence of luminiferous aether; the optical design is applied nearly a century later for gravitational wave detection schema. The perpendicular beam paths set by the beamsplitter, and two end mirrors, create an interference condition of the returning beams at the beamsplitter; disrupted by differential path length changes between the beam paths tracked at the (anti-symmetric) detection port:

\begin{equation}
	P_\mathrm{out} = \frac{P_\mathrm{in}}{2}(1+\mathrm{cos}( 4\pi/\lambda (L_x - L_y))
\end{equation}

Knowing the Michelson will register a diffential shift in length between the arm lengths ($L_x = L_y = L$) with adjusting fringe brightness, the microsocopic length change can also be viewed as a differential phase response of the Michelson imposed by a time-dependent metric perturbation:
...
\begin{figure*}[h!]
	\begin{subfigure}{\includegraphics[width=.45\textwidth,page=2]{INTRO/ifo_configs.pdf}}
 	 ...
 	 \end{subfigure}
  	\hfill
 n 	\caption{The simple 4km Michelson with associated differential arm length response (left is optical schema, right is frequency response)}
  	\label{fig:mich}
\end{figure*}
\FloatBarrier
