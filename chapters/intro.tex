...
``Space-time tells mass how to move; mass tells space-time how to curve" can provide a sufficient summarization of Einstein's theory of general relativity (GR) ~\cite{Misner1973}. Built into this most complete theory of gravity to date, GR makes considerations of high energy astrophyical phenomena (highly massive binary coalescences, spherically assymetric compact objects, etc.) whose fractional mass/energy output generates distortions in space-time known as gravitational waves. 
This can be represented as a perturbation ($h_{\mu \nu}<<1$) in the Minkowski metric tensor defining a local linearized space-time:
\\
$$g_{\mu \nu} = \eta_{\mu \nu} + h_{\mu \nu}$$
\\
Wave-like behavior for $h_{\mu \nu}$ is realized by imposing a transverse traceless (TT) gauge on the above metric perturbation:
\\ 
$$\nabla^2 h_{+} - \frac{1}{c^2} \frac{\partial}{\partial t} h_{+} = 0$$
$$\nabla^2 h_{\times} - \frac{1}{c^2} \frac{\partial}{\partial t} h_{\times} = 0$$
\\
This is merely a simple plane wave solution with two separate transverse polarizations ($h_{+}$ and $h_{\times}$) with a $45^{\circ}$ separation between them. The measurement and analysis of these waves allows the analysis of novel information on the GW progenitors by testing various hypotheses pertaining to the violent dynamics of these systems. September 14th, 2015 the first confirmed gravitational wave detection was made from two black holes coalescing and since then, other detectors (Virgo, KAGRA, etc.) have joined the GW detector network, ushering in an entirely new catalogue of astronomical data.

\newpage
\section{Detector configurations}
\subsection{Interferometry with a Michelson configuration}
Initially used by Michelson and Morley for an experiment intended to support the existence luminiferous aether, the optical design is now applied nearly a century later for gravitational wave detection schema. The perpendicular beam paths set by the beamsplitter and two end mirrors create an interference condition of the returning beams at the beamsplitter; disrupted by differential path length changes between the beam paths tracked at the (anti-symmetric) detection port:

\begin{equation}
	P_\mathrm{out} = \frac{P_\mathrm{in}}{2}(1+\mathrm{cos}( 4\pi/\lambda (L_x - L_y))
\end{equation}

Knowing the Michelson will register a diffential shift in length between the arm lengths (L) with adjusting fringe brightness, the length response can be reframed into a phase response of the Michelson imposed by a time-dependent metric perturbation:
...
Even with state-of-the-art ground motion isolation for terrestrial gravitational detectors and high mass mirrors, current gravitational wave detectors still suffer from occasional misalignment and require sensing and feedback loops to meet mirror alignment requirements.
